...
class VmapWrapper(Wrapper):
    ...
    def __init__(self, env: Env, batch_size: Optional[int] = None, device="cpu"):
        super().__init__(env)
    ...
    def reset(self, rng: jax.Array) -> State:
        ...
        return jax.device_put(reset_fn(rng), device=self.device)

    def step(self, state: State, action: jax.Array) -> State:
        step_fn = jax.vmap(self.env.step)
        return jax.device_put(step_fn(state, action), device=self.device)
