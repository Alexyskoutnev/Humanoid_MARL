...
class Humanoids(PipelineEnv):
    ...
    def step(self, state: State, action: jax.Array) -> State:
        ...
        forward_reward = self._forward_reward_weight * jp.abs(
            jp.sqrt(velocity[:, 0] ** 2 + velocity[:, 1] ** 2)
        )
    ...
    def _wall_penalty(self, pipeline_state: base.State) -> jax.Array:
        ...
        wall_a1_pen = jp.clip(jp.exp(10 * (jp.abs(a1_pos) - 3.5)), 10e-5, 10)
        wall_a2_pen = jp.clip(jp.exp(10 * (jp.abs(a2_pos) - 3.5)), 10e-5, 10)
