...
def train(
    ...
    env = envs.create(
        ...
        backend="spring",
        dummy_dims=network_config.get("dummy_dim") or 0,
    )
    ...
    network_arch = {
        "policy_layers": network_config["policy_network_dim"],
        "value_layers": network_config["value_network_dim"],
    ...
    # create the agent
    if network_config.get("policy_network_dim"):
        policy_layers = network_config.get("policy_network_dim")
    else:
        policy_layers = [
    ...
    if network_config.get("value_network_dim"):
        value_layers = network_config.get("value_network_dim")
    else:
    ...
    for eval_i in range(eval_frequency + 1):
        ...
        for _ in range(num_epochs):
            ...
            # batch_size = 1024
            # num_minibatches = 32
            # unroll_length = 5 (episode length)
            # env.num_envs = 2048
            # num_unrolls = 16
            # td.observation.shape -> [num_unrolls, unroll_length + 1, env.num_envs, env.obs_dim] torch.Size([16, 6, 2048, 244])
            # ========================== Important State INFO  ==========================
