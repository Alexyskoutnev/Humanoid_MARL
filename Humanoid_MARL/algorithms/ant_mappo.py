from typing import Dict, Sequence, Tuple, ClassVar, Optional
import logging
import math
import torch
...
class CriticMAPPO(nn.Module):
    def __init__(self, value_layers: Sequence[int]):
...
class AgentMAPPO(nn.Module):
    def __init__(
        ...
        super(AgentMAPPO, self).__init__()
        self.policy = nn.ModuleList(
            [ActorMAPPO(policy_layers) for _ in range(num_agents)]
        )
        self.value = CriticMAPPO(value_layers)
        self.num_steps = torch.zeros((), device=device)
        self.running_mean = torch.zeros((num_agents, policy_layers[0]), device=device)
        self.running_variance = torch.zeros(
            (num_agents, policy_layers[0]), device=device
        )
    ...
    @torch.jit.export
    def update_normalization(self, observation: torch.Tensor, idx: int) -> None:
        """
        ...
        input_to_old_mean = observation - self.running_mean[idx]
        ...
        self.running_mean[idx] = self.running_mean[idx] + mean_diff
        # Calculate the difference between the current observation and the updated running mean
        input_to_new_mean = observation - self.running_mean[idx]
        ...
        self.running_variance = self.running_variance[idx] + var_diff

    @torch.jit.export
    def normalize(
        self, observation: torch.Tensor, idx: Optional[int] = None
    ) -> torch.Tensor:
        """
        ...
        """
        if idx is not None:
            variance = self.running_variance[idx] / (self.num_steps + 1.0)
            variance = torch.clip(variance, 1e-6, 1e6)
            # Converting to z-score space and clipping to [-5, 5]
            return ((observation - self.running_mean[idx]) / variance.sqrt()).clip(
                -5, 5
            )
        else:
            obs_two = observation.reshape(
                (observation.shape[0], observation.shape[1], self.num_agents, -1)
            )
            for i in range(self.num_agents):
                variance = self.running_variance[i] / (self.num_steps + 1.0)
                variance = torch.clip(variance, 1e-6, 1e6)
                obs_two[:, :, i, :] = (
                    (obs_two[:, :, i, :] - self.running_mean[i]) / variance.sqrt()
                ).clip(-5, 5)
            return obs_two.reshape((obs_two.shape[0], obs_two.shape[1], -1))
    ...
    def get_logits_action(
        ...
        """
        observation = self.normalize(observation, idx)
        logits = self.policy[idx](observation)
    ...
    def loss(self, td: Dict[str, torch.Tensor]):
        ...
        for agent_idx in range(self.num_agents):
            ...
            td_obs_full_state = td["observation"].reshape(
                (td_obs.shape[0], td_obs.shape[1], -1)
            )
            full_state_observation = self.normalize(td_obs_full_state)
            policy_logits = self.policy[agent_idx](observation[:-1])
            # Get the reward, action, logits for the agent at agent_idx
            reward = td["reward"][:, :, agent_idx] * self.reward_scaling

            baseline = self.value(full_state_observation)
            ...
            with torch.no_grad():
                current_agent_baseline = baseline[:, :, agent_idx]
                current_agent_bootstrap_value = bootstrap_value[:, agent_idx]
                vs, advantages = self.compute_gae(
                    truncation=td["truncation"],
                    termination=termination,
                    reward=reward,
                    values=current_agent_baseline,
                    bootstrap_value=current_agent_bootstrap_value,
                )
            ...
            # Value function loss
            v_error = (
                vs - current_agent_baseline
            )  # value expectation - value prediction from network
            ...
            self.loss_dict[f"policy_loss_{agent_idx}"] = policy_loss.item()
            self.loss_dict[f"value_loss_{agent_idx}"] = v_loss.item()
            self.loss_dict[f"entropy_loss_{agent_idx}"] = entropy_loss.item()
            self.loss_dict[f"total_loss_{agent_idx}"] = (
                policy_loss.item() + v_loss.item() + entropy_loss.item()
            )
            loss_actors.append(policy_loss)
            loss_critics.append(v_loss)
            loss_entropy.append(entropy_loss)

        # reduction
        policy_loss = torch.mean(torch.stack(loss_actors))
        v_loss = torch.mean(torch.stack(loss_critics))
        entropy_loss = torch.mean(torch.stack(loss_entropy))
